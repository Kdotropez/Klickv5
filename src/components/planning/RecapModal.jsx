...
const RecapModal = ({ type, employee, shop, days, config, selectedWeek, planning, selectedEmployees, onClose, calculateEmployeeDailyHours, calculateEmployeeWeeklyHours, calculateShopWeeklyHours }) => {
    ...
    const calculateRecapData = () => {
        ...
        if (type === 'employee') {
            return days.map((day, index) => {
                ...
                if (!Array.isArray(slots) || slots.length !== config.timeSlots.length) {
                    return {
                        day: day.name,
                        arrival: '-',
                        pause1: '-',
    ...
    const exportToPDF = () => {
        ...
        if (type === 'employee') {
            const employeeWeeklyHours = calculateEmployeeWeeklyHours(employee, selectedWeek, planning).toFixed(1);
            doc.text(`Récapitulatif individuel pour ${employee} (${employeeWeeklyHours} h) - ${shop}`, 14, 20);
            ...
            doc.autoTable({
                ...
                body: data.map(row => [row.day, row.arrival, row.pause1, row.return1, row.pause2, row.return2, row.exit, row.hours])
                    .concat([['Total général', '', '', '', '', '', '', employeeWeeklyHours]]),
                startY: 40,
                theme: 'grid',
            ...
            doc.text(`Récapitulatif hebdomadaire - ${shop} (${shopWeeklyHours} h)`, 14, 20);
            doc.text(`Semaine du Lundi ${format(new Date(selectedWeek), 'd MMMM', { locale: fr })} au Dimanche ${format(addDays(new Date(selectedWeek), 6), 'd MMMM', { locale: fr })}`, 14, 30);
            let startY = 40;
            data.forEach((dayData, index) => {
                doc.text(dayData.day, 14, startY);
                const dailyTotalHours = dayData.employees.reduce((sum, emp) => sum + parseFloat(emp.hours), 0).toFixed(1);
                doc.autoTable({
            ...
            doc.autoTable({
                body: [['Total semaine', '', '', '', '', '', '', shopWeeklyHours]],
                startY: startY,
                theme: 'grid',
        ...
        <div className="modal-overlay">
            <div className="modal-content">
                ...
                <h3 style={{ fontFamily: 'Roboto, sans-serif', textAlign: 'center' }}>
                    {type === 'employee'
                        ? `Récapitulatif pour ${employee} (${calculateEmployeeWeeklyHours(employee, selectedWeek, planning).toFixed(1)} h)`
                        : `Récapitulatif semaine - ${shop} (${calculateShopWeeklyHours()} h)`}
                </h3>
                ...
                {error ? (
                    ...
                    <table className="recap-table" style={{ fontFamily: 'Inter, sans-serif', width: '100%', borderCollapse: 'collapse' }}>
                        <thead>
                                ...
                                recapData.map((dayData, dayIndex) => (
                                    <React.Fragment key={dayIndex}>
                                        ...
                                        <tr key={`${dayIndex}-total`} style={{ backgroundColor: '#f0f0f0', fontWeight: '700' }}>
                                            ...
                                            <td style={{ border: '1px solid #ddd', padding: '8px' }}></td>
                                            <td style={{ border: '1px solid #ddd', padding: '8px' }}></td>
                                            <td style={{ border: '1px solid #ddd', padding: '8px' }}>{dayData.employees.reduce((sum, emp) => sum + parseFloat(emp.hours), 0).toFixed(1)} h</td>
                                        </tr>
